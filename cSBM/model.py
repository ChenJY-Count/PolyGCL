...
class Model(nn.Module):
    def __init__(self, in_dim, out_dim, K, dprate, dropout, is_bns, act_fn, init_low=2.0, init_high=0.0, val_range=2.0):
        super(Model, self).__init__()

        self.encoder = ChebNetII(num_features=in_dim, hidden=out_dim, K=K, dprate=dprate, dropout=dropout, is_bns=is_bns, act_fn=act_fn, init_low=init_low, init_high=init_high, val_range=val_range)
    ...
    def get_embedding(self, edge_index, feat):
        ...
        tmp_alpha = F.sigmoid(self.alpha)
        h = torch.mul(tmp_alpha, h1) + torch.mul(1 - tmp_alpha, h2)
...
class ChebNetII(torch.nn.Module):
    def __init__(self, num_features, hidden=512, K=10, dprate=0.50, dropout=0.50, is_bns=False, act_fn='relu',init_low=2.0, init_high=0.0, val_range=2.0):
        super(ChebNetII, self).__init__()
        self.lin1 = Linear(num_features, hidden)

        self.prop1 = ChebnetII_prop(K=K,init_low=init_low, init_high=init_high, val_range=val_range)
